...
class ComplaintViewSet(viewsets.ModelViewSet):
        ...
        return Response(
            {
                ...
                "complaint": ComplaintSerializer(
                    complaint_obj, context={"request": request}
                ).data,
            }
        )
    ...
    def assign_complaint(self, request, pk=None):
        ...
        if not officer_id:
            return Response(
                {"error": "officer_id is required."}, status=status.HTTP_400_BAD_REQUEST
            )
        ...
        try:
            ...
            return Response(
                {"error": "Active officer not found."}, status=status.HTTP_404_NOT_FOUND
            )
    ...
    def track(self, request, pk=None):
        complaint_obj = self.get_object()
        if (
            complaint_obj.citizen != request.user
